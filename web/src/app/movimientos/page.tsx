...
const formatDateTime = (value: string) => new Date(value).toLocaleString("es-DO");
  ...
  if (!isAdmin) {
    return (
      <AdminLayout active="Movimientos">
        <p className="text-sm text-slate-500">No tienes permisos para consultar los movimientos de inventario.</p>
      </AdminLayout>
    );
  }
      ...
      <section className="mt-6 rounded-3xl border border-slate-100 bg-white/80 p-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="grid flex-1 gap-4 md:grid-cols-3">
            <div>
              <label className="text-xs uppercase text-slate-400">Tipo de movimiento</label>
              <select
        ...
        <div className="mt-6">
          {loading ? (
            ...
            <p className="text-sm text-slate-400">No se encuentran movimientos con los filtros seleccionados.</p>
          ) : (
                ...
                <tbody>
                  {filteredMovimientos.map((movimiento) => (
                    <tr key={movimiento.id} className="border-t border-slate-100">
                      <td className="px-4 py-2 font-medium text-slate-800">{movimiento.producto}</td>
                      <td className="px-4 py-2">
                        <span
                          className={`rounded-full px-3 py-1 text-xs font-semibold capitalize ${tipoClasses[movimiento.tipoMovimiento] ?? "bg-slate-100 text-slate-600"}`}
                        >
                          {movimiento.tipoMovimiento}
                        </span>
                      </td>
                      <td className="px-4 py-2 text-slate-600">{movimiento.cantidad}</td>
                      <td className="px-4 py-2 text-slate-600">{movimiento.stockAnterior}</td>
                      <td className="px-4 py-2 text-slate-600">{movimiento.stockNuevo}</td>
                      <td className="px-4 py-2 text-slate-600">{formatDateTime(movimiento.fecha)}</td>
                      <td className="px-4 py-2 text-slate-600">{movimiento.usuario}</td>
                      <td className="px-4 py-2 text-slate-600">{movimiento.observacion ?? "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
